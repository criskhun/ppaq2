...
include("../dashboard/navbar.php");
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $sql =  "SELECT * FROM deactiveUser_tbl WHERE id = $id";
        ...
        do {
            try {
                $sql = "INSERT INTO users (email, first_name, midle_name, surname, birthdate, phone, password, date, role, designation)
                ...
                if (isset($_GET["id"])) {
                    ...
                    $sql = "DELETE FROM deactiveUser_tbl WHERE id = $id";
                    if ($conn->query($sql) === TRUE) {
                        $successMessage = "User added correctly";
                        $redirectUrl = "../users/inActiveUsers.php";
